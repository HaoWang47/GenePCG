...
Est_qiu_dfmax=function(df, alpha=0.05, degree_freedom=0, c0=0.25){ # control_var is the k_th variable you want to force to be included in the regression
  require(glmnet)
  require(doMC)
  ...
  for (i in 1 : p){
    ...
    #coef(out.cv, s=out.cv$lambda.min)
    #coef(out.cv, s='lambda.min')
    if (length(which(coef(out.cv, s='lambda.min')[-1]!=0))<=degree_freedom){ # cv
      Coef=coef(out, s=out.cv$lambda.min)[-1]
      Predict = predict(out,XS[, -i],s=out.cv$lambda.min) %>% as.matrix()
      print(paste0("i=",i,", df=",length(which(Coef!=0)), ", df selected by CV"))
    } else{ # dfmax
      Coef=coef(out,s=out$lambda[which.max(out$df==degree_freedom)])[-1]
